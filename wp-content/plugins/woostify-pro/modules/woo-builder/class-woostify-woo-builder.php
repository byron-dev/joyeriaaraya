...
if ( ! class_exists( 'Woostify_Woo_Builder' ) ) {
	...
	class Woostify_Woo_Builder {
		 ...
		 */
		public function product_page_woobuilder() {
			...
			foreach ( $template_id as $k => $v ) {
				...
				if ( ! empty( $include ) ) {
					foreach ( $include as $in ) {
						...
						if ( $is_shop && 'shop-page' === $in ) {
							array_unshift( $include_data, $k );
						} elseif (
							false !== strpos( $in, 'all' ) ||
							( $is_cat && false !== strpos( $in, 'in-cat-' ) && $in_cat_id === $object_id ) ||
							( $is_tag && false !== strpos( $in, 'in-tag-' ) && $in_tag_id === $object_id )
						) {
							array_push( $include_data, $k );
						}
					}
				...
				if ( ! empty( $exclude ) ) {
					foreach ( $exclude as $ex ) {
						...
						if (
							...
							( $is_cat && 'in-cat-all' !== $ex && false !== strpos( $ex, 'in-cat-' ) && $ex_cat_id === $object_id ) ||
							( $is_tag && 'in-tag-all' !== $ex && false !== strpos( $ex, 'in-tag-' ) && $ex_tag_id === $object_id )
						) {
							array_push( $exclude_data, $k );
						}
					}
		...
		public function single_template( $template ) {
			...
			if ( is_singular( 'product' ) && $this->template_exist() ) {
				...
				} elseif ( $this->template_exist( 'woostify_checkout_page' ) && empty( is_wc_endpoint_url( 'order-received' ) ) ) {
				...
				} elseif ( 'activated' === get_option( 'woostify_wc_ajax_product_search' ) || defined( 'WOOSTIFY_PRO_AJAX_PRODUCT_SEARCH' ) ) {
