...
if ( ! class_exists( 'Woostify_Quick_View' ) ) :
	...
	class Woostify_Quick_View {
		...
		public function shop_quick_view() {
			check_ajax_referer( 'shop_quick_view', 'ajax_nonce', false );
				...
				while ( have_posts() ) {
					...
					<div <?php wc_product_class(); ?>>
						...
						if ( class_exists( 'BM_Live_Price' ) ) {
							$res['bm_live_price'] = BM_Live_Price::single_product_price( $product->get_price(), $product );
						}
			...
			$res['review_link'] = get_permalink( $product_id ) . '/#reviews';
